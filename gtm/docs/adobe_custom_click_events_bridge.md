...
GTM config normalizes event names and sends them to GA4

High-level flow

A user interaction triggers an ACDL event (e.g. homepage or main navigation click)

The bridge listens for Adobe Data Layer events using addEventListener

Approved events are pushed into GTM’s dataLayer as custom events (acdl.<eventName>)

A GA4 Event tag fires and sends a normalized GA4 event with shared parameters
...
This ensures events are captured reliably regardless of script load timing

Filters events using an internal allowlist (for example homepage and main nav events)
...
Input: GTM {{Event}} (e.g. acdl.homeHeroBanner)
...
event (string, e.g. homeHeroBanner, mainNavClick)
